self.$__dart_deferred_initializers__=self.$__dart_deferred_initializers__||Object.create(null)
$__dart_deferred_initializers__.current=function(a,b,c,$){var B={
atd(d){var x=0,w=A.l(y.b),v,u,t
var $async$atd=A.h(function(e,f){if(e===1)return A.i(f,w)
while(true)switch(x){case 0:v=A.p("logout",!0)
u=A.A(A.p("sureYouWantLogout",!0),null,null,null,null,null,null,null,null,C.u,null,null)
t=J
x=4
return A.f(A.fF(A.a([A.cS(A.A(A.p("no",!0),null,null,null,null,null,null,null,null,null,null,null),new B.bxj(d),null),A.cS(A.A(A.p("yes",!0),null,null,null,null,null,null,null,null,null,null,null),new B.bxk(d),null)],y.l),C.DB,1,u,d,!0,!1,v),$async$atd)
case 4:x=t.o(f,"OK")?2:3
break
case 2:v=A.aD(d,!1)
x=5
return A.f(A.e7("assets/animations/success_animation.json.zip",d,!1,C.aH,B.yf(d),null,A.p("logoutSuccessfully",!0),v,!0,!1,!0,!1,C.b3).eC(),$async$atd)
case 5:case 3:return A.j(null,w)}})
return A.k($async$atd,w)},
yf(d){var x=0,w=A.l(y.e),v,u
var $async$yf=A.h(function(e,f){if(e===1)return A.i(f,w)
while(true)switch(x){case 0:u=y.e
x=3
return A.f(A.ed(J.br(A.bb("userHelper.9",0),new B.bvq(),u),new B.bvr(),u,y.o),$async$yf)
case 3:if(!f){$.aY().aA(C.a5,"Failed to load the library UserHelper",null,null)
v=!1
x=1
break}x=4
return A.f($.af().np(),$async$yf)
case 4:x=5
return A.f($.bZ().yD(),$async$yf)
case 5:x=6
return A.f($.as().D_(),$async$yf)
case 6:x=7
return A.f(A.a_I(),$async$yf)
case 7:A.b6("userHelper.9")
x=8
return A.f(D.e4().iY(),$async$yf)
case 8:v=!0
x=1
break
case 1:return A.j(v,w)}})
return A.k($async$yf,w)},
bxj:function bxj(d){this.a=d},
bxk:function bxk(d){this.a=d},
bvq:function bvq(){},
bvr:function bvr(){}},A,C,J,D
B=a.updateHolder(c[22],B)
A=c[0]
C=c[2]
J=c[1]
D=c[5]
var z=a.updateTypes([])
B.bxj.prototype={
$0(){A.aD(this.a,!1).b8("Cancel")
return null},
$S:0}
B.bxk.prototype={
$0(){var x=0,w=A.l(y.f),v=this
var $async$$0=A.h(function(d,e){if(d===1)return A.i(e,w)
while(true)switch(x){case 0:A.aD(v.a,!1).b8("OK")
return A.j(null,w)}})
return A.k($async$$0,w)},
$S:2}
B.bvq.prototype={
$1(d){return!0},
$S:33}
B.bvr.prototype={
$2(d,e){return!1},
$S:11};(function inheritance(){var x=a.inheritMany,w=a.inherit
x(A.dH,[B.bxj,B.bxk])
w(B.bvq,A.ca)
w(B.bvr,A.df)})()
var y={l:A.G("B<e>"),o:A.G("U"),e:A.G("K"),b:A.G("@"),f:A.G("~")}}
$__dart_deferred_initializers__["EgjfZmeJAMXWASsJiS3HPo81anY="] = $__dart_deferred_initializers__.current
