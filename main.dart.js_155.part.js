self.$__dart_deferred_initializers__=self.$__dart_deferred_initializers__||Object.create(null)
$__dart_deferred_initializers__.current=function(a,b,c,$){var B={a6E:function a6E(d,e,f,g,h){var _=this
_.c=d
_.d=e
_.e=f
_.f=g
_.a=h},aJk:function aJk(d){this.a=d},
cu5(d,e,f,g,h,i){var x,w,v=null,u=new A.b5(v,y.o),t=A.eZ("assets/icons/delete.webp",v,v,60,v,v,60)
t=A.d0(A.a8(A.a([t,C.zN,d,new B.a6E(i,f,h,u,v)],y.u),C.h,C.j,C.f),v,C.n,v,v,v,C.C)
x=A.n("cancel",!1)
w=A.n("delete",!1)
return A.eT(C.CN,1,x,t,e,C.O,!0,C.q,new B.bIS(e),new B.bIT(u,e),!1,w,g).hx(new B.bIU())},
bIS:function bIS(d){this.a=d},
bIT:function bIT(d,e){this.a=d
this.b=e},
bIU:function bIU(){},
aBD(d){return B.cu4(d)},
cu4(d){var x=0,w=A.l(y.b),v=1,u,t,s,r,q,p,o,n,m
var $async$aBD=A.h(function(e,f){if(e===1){u=f
x=v}while(true)switch(x){case 0:v=3
t=A.ba(null,y.p)
r=A.n("deleteUser",!0)
q=$.au()
p=q.d.ch
p===$&&A.b()
p=p.r.length>0?A.J(A.n("forDeleteUser1",!0),null,null,null,null,null,null,null,null,C.v,null,null):A.z(null,null,null)
x=6
return A.e(B.cu5(A.a8(A.a([p,A.J(A.n("forDeleteUser2",!0),null,null,null,null,null,null,null,null,C.v,null,null)],y.u),C.h,C.j,C.f),d,t,r,25,q.d.cx),$async$aBD)
case 6:s=f
x=J.o(s,!0)?7:8
break
case 7:r=A.aw(d,!1)
p=B.tn(d)
o=A.ai(0,0,0,0,0,8)
x=9
return A.e(A.f4("assets/animations/delete_animation.json.zip",d,!1,C.aS,p,null,A.n("UserDeleted",!0),r,!0,!1,!0,!0,o).fd(),$async$aBD)
case 9:A.pX(d).P1()
case 8:v=1
x=5
break
case 3:v=2
m=u
x=5
break
case 2:x=1
break
case 5:return A.j(null,w)
case 1:return A.i(u,w)}})
return A.k($async$aBD,w)},
tn(d){var x=0,w=A.l(y.e),v,u,t,s
var $async$tn=A.h(function(e,f){if(e===1)return A.i(f,w)
while(true)switch(x){case 0:s=y.e
x=3
return A.e(A.fe(J.bW(A.bp("userHelper.8",0),new B.bIO(),s),new B.bIP(),s,y.E),$async$tn)
case 3:if(!f){$.b1().aE(C.a2,"Failed to load the library UserHelper",null,null)
v=!1
x=1
break}u=$.au()
x=4
return A.e(u.Fx(),$async$tn)
case 4:s=u.d.ch
s===$&&A.b()
s=s.w
t=y.w
x=5
return A.e(A.C2(A.ar(new A.aY(s,A.x(s).i("aY<1>")),!0,t),new B.bIQ(d)),$async$tn)
case 5:s=u.d.ch
s===$&&A.b()
x=6
return A.e(A.C2(A.ar(s.r,!0,t),new B.bIR()),$async$tn)
case 6:x=7
return A.e(A.a3H(),$async$tn)
case 7:A.bi("userHelper.8")
x=8
return A.e(D.e4().AO(),$async$tn)
case 8:A.bi("userHelper.8")
x=9
return A.e(D.e4().AL(u.d),$async$tn)
case 9:A.bi("userHelper.8")
x=10
return A.e(D.e4().ic(),$async$tn)
case 10:v=!0
x=1
break
case 1:return A.j(v,w)}})
return A.k($async$tn,w)},
bIO:function bIO(){},
bIP:function bIP(){},
bIQ:function bIQ(d){this.a=d},
bIN:function bIN(d,e){this.a=d
this.b=e},
bIR:function bIR(){},
bIM:function bIM(d){this.a=d}},A,C,F,J,E,D
B=a.updateHolder(c[24],B)
A=c[0]
C=c[2]
F=c[67]
J=c[1]
E=c[65]
D=c[5]
B.a6E.prototype={
p(d){var x=this,w=null,v=A.z(w,x.e,w),u=A.ce(A.n("verifyPhone",!0),w,!1,!1,w,!1,!1,1,w,16,""),t=x.c
t=C.d.q(t,"-")?t.split("-")[1]:""
return A.a8(A.a([v,u,A.k5(w,F.b3z(w,x.d,t,w,w,w,!0,!1,!0,w,new B.aJk(x)),x.f)],y.u),C.aB,C.j,C.f)}}
var z=a.updateTypes([])
B.aJk.prototype={
$1(d){var x=this.a
if(d!==x.c)return A.n("noMatchPhoneNumbers",!0)
if(x.d.a.c==="")return A.n("shortPhone",!0)
return null},
$S:910}
B.bIS.prototype={
$0(){A.aw(this.a,!1).aZ(null)
return null},
$S:0}
B.bIT.prototype={
$0(){var x=this.a
if(x.gV()==null||!x.gV().lo())return
A.aw(this.b,!1).aZ(!0)},
$S:3}
B.bIU.prototype={
$0(){return A.mO()},
$S:2}
B.bIO.prototype={
$1(d){return!0},
$S:78}
B.bIP.prototype={
$2(d,e){return!1},
$S:16}
B.bIQ.prototype={
$1(d){return this.arI(d)},
arI(d){var x=0,w=A.l(y.F),v=this,u
var $async$$1=A.h(function(e,f){if(e===1)return A.i(f,w)
while(true)switch(x){case 0:u=$.au().d.ch
u===$&&A.b()
x=u.w.h(0,d)===1?2:3
break
case 2:x=4
return A.e(J.bW(A.bp("managerHelper.6",0),new B.bIN(d,v.a),y.e),$async$$1)
case 4:case 3:return A.j(null,w)}})
return A.k($async$$1,w)},
$S:82}
B.bIN.prototype={
$1(d){var x=0,w=A.l(y.e),v,u=this
var $async$$1=A.h(function(e,f){if(e===1)return A.i(f,w)
while(true)switch(x){case 0:A.bi("managerHelper.6")
x=3
return A.e(E.bPL().PJ($.au().d.c,u.a,u.b),$async$$1)
case 3:v=f
x=1
break
case 1:return A.j(v,w)}})
return A.k($async$$1,w)},
$S:7}
B.bIR.prototype={
$1(d){return this.arH(d)},
arH(d){var x=0,w=A.l(y.F)
var $async$$1=A.h(function(e,f){if(e===1)return A.i(f,w)
while(true)switch(x){case 0:x=2
return A.e(J.bW(A.bp("managerHelper.6",0),new B.bIM(d),y.e),$async$$1)
case 2:return A.j(null,w)}})
return A.k($async$$1,w)},
$S:82}
B.bIM.prototype={
$1(d){var x=0,w=A.l(y.e),v,u=this
var $async$$1=A.h(function(e,f){if(e===1)return A.i(f,w)
while(true)switch(x){case 0:A.bi("managerHelper.6")
x=3
return A.e(E.bPL().Pz(u.a,"","",""),$async$$1)
case 3:v=f
x=1
break
case 1:return A.j(v,w)}})
return A.k($async$$1,w)},
$S:7};(function inheritance(){var x=a.inherit,w=a.inheritMany
x(B.a6E,A.R)
w(A.cv,[B.aJk,B.bIO,B.bIQ,B.bIN,B.bIR,B.bIM])
w(A.cX,[B.bIS,B.bIT,B.bIU])
x(B.bIP,A.di)})()
A.cG(b.typeUniverse,JSON.parse('{"a6E":{"R":[],"d":[]}}'))
var y={u:A.A("C<d>"),o:A.A("b5<lj>"),F:A.A("aG"),E:A.A("T"),w:A.A("f"),e:A.A("I"),b:A.A("@"),p:A.A("zn?")}}
$__dart_deferred_initializers__["Y2fMGIKmoD/jAyf2K7vfmD7SNJo="] = $__dart_deferred_initializers__.current
