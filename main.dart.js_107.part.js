self.$__dart_deferred_initializers__=self.$__dart_deferred_initializers__||Object.create(null)
$__dart_deferred_initializers__.current=function(a,b,c,$){var B={
a3T(d,e,f,g){return B.cvK(d,!0,f,g)},
cvK(d,e,f,g){var w=0,v=A.l(x.w),u,t,s,r,q,p,o,n
var $async$a3T=A.h(function(h,i){if(h===1)return A.i(i,v)
while(true)switch(w){case 0:n={}
if(d.b==null){u=""
w=1
break}n.a=""
t=g&&f!=null
s=x.e
w=t?3:5
break
case 3:t=A.aw(f,!1)
r=$.aCx()
w=6
return A.e(A.f4(y.c,f,!1,C.aS,r.CI(d).ac(0,new B.bKq(n),s),null,"",t,!0,!0,!0,!1,C.b9).fd(),$async$a3T)
case 6:t=r
w=4
break
case 5:r=$.aCx()
w=7
return A.e(r.CI(d).ac(0,new B.bKr(n),s),$async$a3T)
case 7:t=r
case 4:d.f=E.bId(n.a).h(0,"signature")
s=d.e
r=$.au()
q=r.d.c
p=d.c.a
o=d.r
o=A.N("MM-yyyy",null).M(o)
t.Tu(C.c.m(d.gal5()),d,s,o,q,p)
u=n.a
w=1
break
case 1:return A.j(u,v)}})
return A.k($async$a3T,v)},
bKq:function bKq(d){this.a=d},
bKr:function bKr(d){this.a=d},
c_p(d,e,f){return new B.ajr(f,!0,e,null)},
ajr:function ajr(d,e,f,g){var _=this
_.c=d
_.d=e
_.f=f
_.a=g},
bbX:function bbX(d,e){this.a=d
this.b=e},
bbW:function bbW(d){this.a=d},
bbY:function bbY(d,e){this.a=d
this.b=e},
bM1(d,e){var w=0,v=A.l(x.v),u
var $async$bM1=A.h(function(f,g){if(f===1)return A.i(g,v)
while(true)switch(w){case 0:u=e.c0(0,"\n")
w=2
return A.e($.c8Y().yZ(0,d+"\n"+u,null,null),$async$bM1)
case 2:return A.j(null,v)}})
return A.k($async$bM1,v)}},A,D,C,J,E
B=a.updateHolder(c[46],B)
A=c[0]
D=c[147]
C=c[2]
J=c[1]
E=c[77]
B.ajr.prototype={
p(d){var w=this.f
if(w==null)w=A.aM(D.Ea,null,null,null,24)
return A.d8(w,C.q,new B.bbX(this,d),1)},
Jv(){var w=0,v=A.l(x.q),u,t=this,s
var $async$Jv=A.h(function(d,e){if(d===1)return A.i(e,v)
while(true)switch(w){case 0:s=A.bj(x.w)
w=3
return A.e(A.fp(J.dN(t.c,new B.bbY(t,s),x.A).d2(0),x.F),$async$Jv)
case 3:u=s
w=1
break
case 1:return A.j(u,v)}})
return A.k($async$Jv,v)},
gauJ(){var w=A.n("ShareReceiptUser",!0),v=$.au(),u=v.d.CW
w=A.aB(w,"USERNAME",u)
u=J.hI(this.c).d.b
return A.aB(w,"BUSINESS_NAME",u)}}
var z=a.updateTypes([])
B.bKq.prototype={
$1(d){this.a.a=d
return!0},
$S:37}
B.bKr.prototype={
$1(d){this.a.a=d
return!0},
$S:37}
B.bbX.prototype={
$0(){return this.ar7()},
ar7(){var w=0,v=A.l(x.v),u,t=this,s,r,q,p
var $async$$0=A.h(function(d,e){if(d===1)return A.i(e,v)
while(true)switch(w){case 0:p={}
p.a=A.bj(x.w)
s=t.b
r=A.aw(s,!1)
q=t.a
w=3
return A.e(A.f4(y.c,s,!1,C.aS,q.Jv().ac(0,new B.bbW(p),x.e),null,"",r,!0,!0,!0,!1,C.b9).fd(),$async$$0)
case 3:r=p.a
if(r.gaV(r)){new A.be(A.n("thereIsProblem",!0),C.z,C.B,s,null).aY()
w=1
break}w=4
return A.e(B.bM1(q.gauJ(),p.a),$async$$0)
case 4:case 1:return A.j(u,v)}})
return A.k($async$$0,v)},
$S:2}
B.bbW.prototype={
$1(d){this.a.a=d
return!0},
$S:918}
B.bbY.prototype={
$1(d){return this.ar8(d)},
ar8(d){var w=0,v=A.l(x.F),u=this,t,s
var $async$$1=A.h(function(e,f){if(e===1)return A.i(f,v)
while(true)switch(w){case 0:w=d.f!=null?2:4
break
case 2:t=d.gvO(d)
t=A.aB(t,"\n","")
s=A.aB(t," ","")
w=3
break
case 4:w=d.b!=null?5:7
break
case 5:w=8
return A.e(B.a3T(d,!0,null,!1),$async$$1)
case 8:w=6
break
case 7:f=""
case 6:s=f
case 3:s=A.aB(s," ","")
if(s!=="")u.b.u(0,s)
return A.j(null,v)}})
return A.k($async$$1,v)},
$S:919};(function inheritance(){var w=a.inheritMany,v=a.inherit
w(A.cv,[B.bKq,B.bKr,B.bbW,B.bbY])
v(B.ajr,A.R)
v(B.bbX,A.cX)})()
A.cG(b.typeUniverse,JSON.parse('{"ajr":{"R":[],"d":[]}}'))
var y={c:"assets/animations/success_animation.json.zip"}
var x={A:A.A("ac<aG>"),F:A.A("aG"),q:A.A("dg<f>"),w:A.A("f"),e:A.A("I"),v:A.A("~")};(function constants(){D.Ea=new A.bU(58189,"MaterialIcons",null,!1)})()}
$__dart_deferred_initializers__["HM1viafSr3v9ByJBJIjIARXw3i8="] = $__dart_deferred_initializers__.current
