self.$__dart_deferred_initializers__=self.$__dart_deferred_initializers__||Object.create(null)
$__dart_deferred_initializers__.current=function(a,b,c,$){var B={
Nb(d,e,f){var x=0,w=A.l(y.b),v,u
var $async$Nb=A.h(function(g,h){if(g===1)return A.i(h,w)
while(true)switch(x){case 0:x=6
return A.e(A.fs(d,A.bp("invoicePage",0)),$async$Nb)
case 6:x=h===!0?3:5
break
case 3:u=A.jN(new B.bL9(f,!0),null,y.a)
x=7
return A.e(A.aw(d,!1).fS(u),$async$Nb)
case 7:v=h
x=1
break
x=4
break
case 5:new A.be(A.n("thereIsProblem",!0),C.z,C.B,d,null).aY()
case 4:case 1:return A.j(v,w)}})
return A.k($async$Nb,w)},
bL9:function bL9(d,e){this.a=d
this.b=e}},A,D,C
B=a.updateHolder(c[59],B)
A=c[0]
D=c[26]
C=c[2]
var z=a.updateTypes(["HG(D)"])
B.bL9.prototype={
$1(d){A.bi("invoicePage")
return D.cik(this.b,this.a)},
$S:z+0};(function inheritance(){var x=a.inherit
x(B.bL9,A.cv)})()
var y={b:A.A("@"),a:A.A("T?")}}
$__dart_deferred_initializers__["cLxUFcVbkfByODFjpI/YyY1G8/w="] = $__dart_deferred_initializers__.current
